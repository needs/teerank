#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
#include <limits.h>
#include <time.h>
#include <errno.h>
#include <dirent.h>
#include <string.h>

#include "config.h"
#include "server.h"

static int is_server_offline(struct server_meta *meta, long days)
{
	assert(meta != NULL);
	assert(days >= 0);

	return (time(NULL) - meta->last_seen) / (3600 * 24) >= days;
}

static int number_of_days(char *str)
{
	long ret;

	assert(str != NULL);

	errno = 0;
	ret = strtol(str, NULL, 10);
	if (errno != 0) {
		perror(str);
		exit(EXIT_FAILURE);
	}

	return ret;
}

int main(int argc, char **argv)
{
	char path[PATH_MAX];
	struct dirent *dp;
	DIR *dir;
	long days;
	int dry_run = 0;

	load_config();

	if (argc != 2 && argc != 3) {
		fprintf(stderr, "usage: %s <days> [dry-run]\n", argv[0]);
		return EXIT_FAILURE;
	}

	days = number_of_days(argv[1]);

	if (argc == 3) {
		if (strcmp(argv[2], "dry-run")) {
			fprintf(stderr, "The optional third argument must be equal to 'dry-run'\n");
			return EXIT_FAILURE;
		}
		dry_run = 1;
	}

	sprintf(path, "%s/servers", config.root);
	if (!(dir = opendir(path)))
		return perror(path), EXIT_FAILURE;

	while ((dp = readdir(dir))) {
		struct server_meta meta;

		if (!strcmp(".", dp->d_name) || !strcmp("..", dp->d_name))
			continue;
		read_server_meta(&meta, dp->d_name);

		if (is_server_offline(&meta, days)) {
			if (dry_run)
				printf("'%s' would have been removed\n", dp->d_name);
			else
				remove_server(dp->d_name);
		}
	}

	closedir(dir);

	return EXIT_FAILURE;
}
